...
class Task2:
    ...
    def get_suffix_A(self, step: int) -> str:
        ...
        """
        if 'multi_level' not in self.strategy:
            suffix = self.prompt[self.strategy + '_A']
    ...
    def get_suffix_C(self, step: int, substring_len: int) -> str:
        ...
        if step % 2 == 0:
            if 'reasoning' not in self.strategy:
                suffix = self.prompt['action_direct_C_1']
            else:
                suffix = self.prompt['action_reasoning_C_1']
    ...
    def generate_step(self, step: int, max_result_token: int, max_content_token: int) -> tuple:
        ...
        if step % 2 == 0:
            ...
            if '结束会话' in generate_result:
            ...
            elif '继续搜索' in generate_result:

                if 'reasoning' not in self.strategy:
                    suffix_C = self.prompt['action_direct_C_2']
                else:
                    suffix_C = self.prompt['action_reasoning_C_2']  # 要求
                ...
                start_index = generate_result.find("[") + 1 if generate_result.find("[") != -1 else generate_result.find("\"") + 1
                end_index = generate_result.find("]") if generate_result.find("]") != -1 else generate_result.rfind("\"")
        ...
        if step % 2 == 0:
            if '结束会话' in generate_result:
                ...
                if 'reasoning' not in self.strategy:
                    ...
                    suffix_C = self.prompt['action_reasoning_C_2']
